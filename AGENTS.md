...
## One-liner
Next.js 16 app (App Router, JS, not TS) helping Filipinos navigate government paperwork. Content-driven guides in Markdown + MongoDB for user data + Docker Compose deploy.
...
- **`app/src/data/guides/`** — 25 Markdown files with YAML frontmatter (no CMS). Guides parsed via gray-matter server-side. Frontmatter fields: title, shortTitle, slug, description, lastUpdated, category, agency, difficulty, estimatedTime, costRange, tags, aliases, checklist
- **`app/src/models/`** — Mongoose models: User, GovernmentOffice, GuideStats, OfficeReport, RateLimit
- **`app/src/lib/`** — MongoDB connect, auth (NextAuth + Google OAuth + credentials with JWT), rate-limit, metrics (prom-client), mail, seed
- **`infra/terraform/`** + **`infra/ansible/`** — provisioning; secrets managed via Ansible Vault
- **`docker/compose/`** — full stack: app, mongodb, nginx, watchtower, Prometheus, Grafana, Alertmanager, node-exporter, cadvisor, backup (rclone to Cloudflare R2)
- **CI** — `.github/workflows/pipeline.yml`: lint → Docker build → Trivy scan (CRITICAL/HIGH only) → push to ghcr.io
