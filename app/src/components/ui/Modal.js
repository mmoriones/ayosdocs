...
export default function Modal({
  ...
  return (
    <div className={`fixed inset-0 z-[200] flex items-center justify-center p-4 pointer-events-none ${className}`}>
      <div
        className={`absolute inset-0 bg-ctp-crust/20 backdrop-blur-[2px] pointer-events-auto animate-in fade-in duration-300 ${overlayClassName}`}
        onClick={closeOnOverlay ? onClose : undefined}
      />

      <div className={`relative w-full ${sizes[size]} bg-ctp-mantle rounded-2xl shadow-xl border border-ctp-surface1 pointer-events-auto animate-in fade-in zoom-in-95 duration-300 ${contentClassName}`}>
        {title && (
