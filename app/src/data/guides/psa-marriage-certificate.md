---
title: How to Get a PSA Marriage Certificate in the Philippines
shortTitle: PSA Marriage Certificate
slug: psa-marriage-certificate
description: Secure your official marriage record for name changes and spousal benefits.
lastUpdated: 2026-05-14
category: Civil Registry
agency: PSA
difficulty: Easy
estimatedTime: Same Day to Several Weeks
costRange: Under ₱500
tags:
- Marriage Certificate
- PSA
- Marriage Contract
- Civil Registry
- Philippines
- PSA Marriage Certificate
aliases:
- Marriage Contract
- PSA Marriage Contract
- Certificate of Marriage
- PSA Marriage Certificate
checklist:
- Prepare valid IDs
- Gather marriage details
- Choose request method
- Pay processing fees
- Claim or receive certificate
---

## 1. What is a PSA Marriage Certificate?

A **PSA Marriage Certificate** is an official civil registry document issued by the Philippine Statistics Authority (PSA) that serves as proof that a marriage has been legally registered in the Philippines.

It contains important information such as:

* Names of both spouses
* Date of marriage
* Place of marriage
* Name of solemnizing officer
* Witness information
* Registration details

The document is commonly required for:

* Passport applications
* Visa and immigration processing
* Change of surname
* SSS, PhilHealth, and Pag-IBIG updates
* Bank and insurance transactions
* Employment requirements
* Legal proceedings
* Spousal benefits claims

Many Filipinos also refer to this document as a “Marriage Contract.”

---

## 2. Who Needs This?

You may need a PSA Marriage Certificate if you are:

- Newly married
- Updating your civil status
- Changing your surname
- Applying for a passport or visa
- Processing immigration requirements
- Applying for spousal benefits
- Filing insurance or inheritance claims
- Updating government records
- Applying for loans or joint accounts

The PSA-issued copy is the version most commonly required by government agencies and foreign embassies.

---

## 3. Benefits of Having a PSA Marriage Certificate

### Official Proof of Marriage

The document serves as government-recognized proof that your marriage is legally registered.

### Required for Government Transactions

Many agencies require the PSA copy when updating records or processing benefits.

### Useful for Immigration and Visa Applications

Foreign embassies and immigration offices commonly request the document.

### Needed for Name Changes

Married individuals updating IDs and records may need the certificate.

### Accepted for Legal and Financial Transactions

Banks, insurers, and courts may require it for verification purposes.

---

## 4. Requirements

### Basic Information Needed

When requesting a copy, prepare the following details:

* Complete name of husband
* Complete maiden name of wife
* Date of marriage
* Place of marriage
* Purpose of request
* Number of copies needed



### Valid IDs

Applicants are usually required to present valid government-issued IDs.

Accepted IDs may include:

* Passport
* Driver’s License
* National ID (PhilSys ID)
* UMID
* Postal ID
* PRC ID
* Voter’s ID

### Additional Documents (if applicable)

Some requests may require:

* Authorization letter
* Special Power of Attorney (SPA)
* Proof of relationship
* Supporting civil registry documents

This may apply if requesting on behalf of another person.

---

## 5. Types of Requests

### PSA Copy Request

This is the most commonly required version issued by the PSA on security paper.

### Local Civil Registry Copy

A certified local copy may be obtained from the city or municipal civil registrar where the marriage was registered.

This may be useful if:

* The marriage is newly registered
* PSA records are not yet available
* Corrections are needed



### Digital PSA E-Certificate

The PSA also offers digital e-certificates through selected online services.

Digital copies may include QR verification and downloadable PDF access.

---

## 6. Application Methods

Applicants may request a PSA Marriage Certificate through several methods.

### Online Request

Applicants may order online through PSA-authorized delivery services.

This option is convenient for:

* Home delivery
* Overseas applicants
* Busy schedules
* Remote requests

### Walk-In Request

Applicants may visit:

* PSA CRS Outlets
* Selected PSA service centers

Walk-in requests may require online appointments depending on branch policies.

### Authorized Representative

Another person may request the document on your behalf if proper authorization is provided.

---

## 7. Step-by-Step Request Process

### Method 1 — Online Request

### Step 1 – Prepare Your Information

Gather the required marriage details including:

* Full names of spouses
* Date of marriage
* Place of marriage

Ensure all details are accurate.

---

### Step 2 – Access the PSA Online Service

Visit the PSA-authorized online request platform.

Provide:

* Delivery information
* Contact details
* Marriage information
* Purpose of request

---

### Step 3 – Pay the Request Fee

Payment methods may include:

* GCash
* Maya
* Credit/debit cards
* Online banking
* Bayad Centers
* Convenience stores

Keep screenshots and payment confirmations.

---

### Step 4 – Wait for Processing and Delivery

After payment confirmation:

* PSA processes the request
* Delivery schedules vary by location
* Metro Manila deliveries are usually faster

Applicants may receive tracking updates depending on the service provider.

---

## Method 2 — Walk-In Request

### Step 5 – Schedule an Appointment (if required)

Some PSA branches require online appointment booking before visiting.

---

### Step 6 – Visit a PSA CRS Outlet

Bring:

* Valid ID
* Reference number (if applicable)
* Payment receipt
* Authorization documents if needed

---

### Step 7 – Fill Out the Request Form

Provide the required marriage details carefully.

Errors in names or dates may delay processing.

---

### Step 8 – Pay the Processing Fee

Proceed to the cashier after verification.

Keep the official receipt for claiming purposes.

---

### Step 9 – Claim Your PSA Marriage Certificate

If approved:

* Wait for your queue number
* Verify all printed information
* Report errors immediately

Some branches release documents on the same day.

---

## 8. Fees

| Item | Estimated Cost |
|---|---|
| Walk-In PSA Copy | ₱155–₱210 |
| Online Request with Delivery | ₱330–₱500 |
| Additional Copies | Additional fees apply |

Fees may vary depending on delivery location and service providers.

---

## 9. Processing Time

Typical processing timelines include:

| Request Type | Estimated Processing Time |
|---|---|
| Walk-In Request | Same day |
| Metro Manila Delivery | 2–5 working days |
| Provincial Delivery | 3–10 working days |
| Newly Registered Marriages | Several weeks to months |

Newly married couples may need to wait before the PSA copy becomes available because the marriage record must first be endorsed by the Local Civil Registry Office to the PSA.

Some couples report waiting around 2–6 months before PSA records become available.

---

## 10. Common Problems Encountered

Applicants may experience:

- “No Record Found” results
- Delayed PSA endorsement
- Incorrect names or details
- Delayed delivery
- Missing records
- Encoding errors
- Website payment issues

Recently married couples commonly experience delays because records may not yet be transmitted to the PSA database.

---

## 11. Common Mistakes to Avoid

- Entering incorrect marriage dates
- Using incomplete names
- Requesting too early after the wedding
- Forgetting authorization documents
- Losing payment receipts
- Using unofficial websites

Always double-check all encoded information before submission.

---

## 12. Tips for Faster Processing

* Ensure names exactly match marriage records
* Keep digital copies of receipts
* Request multiple copies if needed
* Wait several weeks before requesting newly registered marriages
* Follow up with the Local Civil Registry if records are unavailable
* Use authorized PSA request services only

Some couples report faster availability when the Local Civil Registry performs early endorsement to PSA.

---

## 13. Frequently Asked Questions

### Is a Marriage Certificate the same as a Marriage Contract?

Yes. In the Philippines, people commonly use the terms interchangeably.

### How long before a PSA Marriage Certificate becomes available?

Availability may take several weeks to several months after the wedding depending on local registry endorsement and PSA processing.

### Can I request online?

Yes. PSA-authorized online delivery services are available.

### Can someone else request my certificate?

Yes, provided they have authorization documents and valid identification.

### What happens if PSA has no record of my marriage?

You may need to coordinate with the Local Civil Registry Office where the marriage was registered for endorsement or correction.

### Is the PSA digital e-certificate valid?

Yes. PSA digital e-certificates include verification systems and are legally recognized.
